...
### [Umia Finance](https://ethskills.com/umia/SKILL.md) — Agentic Venture Platform
Full-stack platform: token launches (Tailored Auctions / Uniswap CCA), governance (Decision Markets / futarchy), legal (MetaLex BORG / Cayman SPC), noncustodial treasury, zkTLS audience gating, ERC-4337 smart wallet.
- Tailored Auctions: price is non-decreasing (CCA), proceeds → noncustodial treasury, Uniswap V4 pool seeds at clearing price.
- Decision Markets: markets priced not polled — TWAP settlement, futarchy-style. Outcomes are legally binding board decisions.
- `umia evaluate` is non-binding. `umia venture init` creates legal entity + deploys contracts. No legal fees.
- UmiaValidationHook: monotonic step access, three verification paths (zkTLS, EIP-712 permit, hookData). `setCCA()` is one-time irreversible.
- Smart wallet: ERC-4337 (Kernel + Privy), Pimlico paymaster. Smart account address ≠ signer EOA address.

Sub-skills: `umia/listing-tracks/`, `umia/tailored-auctions/`, `umia/zktls/`, `umia/decision-markets/`, `umia/legal-framework/`, `umia/treasury/`, `umia/umia-token/`, `umia/cli/`, `umia/smart-wallet/`, `umia/validation-hook/`, `umia/security/`
...
| Launching on Umia | `umia/`, `umia/listing-tracks/`, `umia/cli/` |
| Building Umia auction gating | `umia/tailored-auctions/`, `umia/zktls/`, `umia/validation-hook/` |
| Umia governance / treasury | `umia/decision-markets/`, `umia/treasury/` |
| Umia legal structure | `umia/legal-framework/` |
| Integrating Umia smart wallet | `umia/smart-wallet/` |
